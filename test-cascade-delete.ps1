...
try {
    ...
    Write-Host "⚠️ Profile might already exist or failed to create: $($_.Exception.Message)" -ForegroundColor Yellow
}
...
try {
    ...
    Write-Host "⚠️ Hostel data might already exist or failed to create: $($_.Exception.Message)" -ForegroundColor Yellow
}
...
try {
    ...
    Write-Host "⚠️ Failed to create test request: $($_.Exception.Message)" -ForegroundColor Yellow
}
...
try {
    ...
    if ($userProfile) {
        ...
        Write-Host "⚠️ No profile data found for test user" -ForegroundColor Yellow
    }
} catch {
...
try {
    ...
    if ($userHostel) {
        ...
        Write-Host "⚠️ No hostel data found for test user" -ForegroundColor Yellow
    }
} catch {
...
try {
    ...
    if ($userRequests) {
        Write-Host "Request data found for test user ($($userRequests.Count) requests)" -ForegroundColor Green
    } else {
...
Write-Host "⚠️ This will delete the user and ALL associated data!" -ForegroundColor Red
...
try {
    ...
    if ($deletedUser) {
        Write-Host "❌ User account still exists!" -ForegroundColor Red
    } else {
        Write-Host "✅ User account successfully deleted" -ForegroundColor Green
    }
} catch {
...
try {
    ...
    $userProfile = $profileCheck.profiles | Where-Object { $_.email -eq $testEmail }
    if ($userProfile) {
        Write-Host "❌ Profile data still exists!" -ForegroundColor Red
    } else {
        Write-Host "✅ Profile data successfully deleted" -ForegroundColor Green
    }
} catch {
    Write-Host "❌ Error checking profile data: $($_.Exception.Message)" -ForegroundColor Red
}
...
try {
    ...
    $userHostel = $hostelCheck.students | Where-Object { $_.email -eq $testEmail }
    if ($userHostel) {
        Write-Host "❌ Hostel data still exists!" -ForegroundColor Red
    } else {
        Write-Host "✅ Hostel data successfully deleted" -ForegroundColor Green
    }
} catch {
    Write-Host "❌ Error checking hostel data: $($_.Exception.Message)" -ForegroundColor Red
}
...
try {
    ...
    $userRequests = $requestsCheck.requests | Where-Object { $_.email -eq $testEmail }
    if ($userRequests) {
        Write-Host "Request data still exists! ($($userRequests.Count) requests)" -ForegroundColor Red
    } else {
        Write-Host "Request data successfully deleted" -ForegroundColor Green
    }
} catch {
    Write-Host "❌ Error checking request data: $($_.Exception.Message)" -ForegroundColor Red
}
